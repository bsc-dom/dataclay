...
from dataclay.exceptions import DataClayException, ObjectNotRegisteredError
...
def test_alias_already_exists(client):
    ...
    assert "Alias admin/test_same_alias_error already exist" in str(excinfo.value)
...
async def test_alias_already_exists_async(client):
    ...
    assert "Alias admin/test_same_alias_error_async already exist" in str(excinfo.value)


def test_alias_does_not_exist(client):
    with pytest.raises(DataClayException) as excinfo:
        Person.get_by_alias("test_alias_does_not_exist_error")
    assert "Alias None/test_alias_does_not_exist_error does not exists" in str(excinfo.value)
...
async def test_alias_does_not_exist_async(client):
    ...
    assert "Alias None/test_alias_does_not_exist_error_async does not exists" in str(excinfo.value)
