...
class AliasRepeatTest(unittest.TestCase):
    ...
    def test(self):
        ...
        """
        from model.classes import Person, Nickname
        ...
        worked = True
        try:
            # test object of same class with same alias cannot be stored
            object2 = Person(name='Alice', age=86)
            object2.make_persistent(alias)
            worked = False
        except:
            pass
        finally:
            try:
                # test object of different class with same alias cannot be registered either
                object2 = Nickname(nickname='foo')
                object2.make_persistent(alias)
                worked = False
            except:
                pass
