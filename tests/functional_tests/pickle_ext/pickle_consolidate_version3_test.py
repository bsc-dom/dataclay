...
class PickleConsolidate3Test(unittest.TestCase):
    ...
    def test(self):
        ...
        # Modifying web page to version and consolidate     
        self.assertEqual(len(web_site_version.pages[0].external_links), 0)
        
        new_ws = WebSite(host)
        
        # FIXME: New WebSite is consolidated just if is maked Persistent before
        new_ws.make_persistent()
        web_site_version.pages[0].add_link(new_ws)        
        
        self.assertEqual(len(web_site_version.pages[0].external_links), 1)
        self.assertEqual(len(web_site.pages[0].external_links), 0)

        web_site.consolidate_version(unloaded_version_info)
        
        self.assertEqual(len(web_site.pages[0].external_links), 1)
