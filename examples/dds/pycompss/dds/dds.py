...
class DDS:  # pylint: disable=too-many-public-methods
    ...
    def load(self, iterator, num_of_parts=10, paac=False, persistent=False):
        ...
        for size in chunk_sizes:
            ...
            print("¬¬¬¬¬¬¬¬¬¬¬¬¬%s¬¬¬¬¬¬¬¬¬¬¬¬", _partition_loader.get_iterable())
            self.partitions.append(_partition_loader)
            start = end

        return self

    def load_file(self, file_path, chunk_size=1024, worker_read=False, persistent=False):
        ...
        if worker_read:
            ...
            with open(file_path, "r") as file_path_fd:  # pylint: disable=W1514
                ...
                while chunk:
                    ...
                    _partition_loader.make_persistent()
                    self.partitions.append(_partition_loader)
                    chunk = file_path_fd.read(chunk_size)
    ...
    def load_text_file(
        self, file_name, chunk_size=1024, in_bytes=True, strip=True, persistent=False
    ):
        ...
        for _p in func(file_name, chunk_size, strip=strip):
            ...
            if(persistent):partition_loader.make_persistent()
            self.partitions.append(partition_loader)

        return self

    def load_files_from_dir(self, dir_path, num_of_parts=-1, persistent=False):
        ...
        for size in partition_sizes:
            ...
            for file_name in files[start:end]:
                file_path = os.path.join(dir_path, file_name,persistent=persistent)
    ...
    def load_pickle_files(self, dir_path, persistent=False):
        ...
        files = sorted(os.listdir(dir_path))
        for _f in files:
            file_name = os.path.join(dir_path, _f,persistent=persistent)
    ...
    def union(self,persistent=False, *args,):
        ...
        return DDS().load(current, num_of_parts=-1, persistent=persistent)
    ...
    def distinct(self,persistent=False):
        ...
        return (
            ...
            .reduce_by_key(lambda x, _: x, persistent=persistent)
            .map(lambda x: x[0])
        )

    def count_by_value(self, arity=2, as_dict=True, as_fo=False, persistent=False):
        ...
        return DDS().load(new_partitions, -1, persistent=persistent)
    ...
    def partition_by(
        self, partitioner_func=default_hash, num_of_partitions=-1, persistent=False
    ):
        ...
        return (
            DDS()
            .load(future_partitions, -1, True, persistent=persistent)
    ...
    def join(self, other, num_of_partitions=-1,persistent=False):
        """Join DDS objects.
        ...
        return (
            buf_a.union(buf_b,persistent=persistent)
            .group_by_key(num_of_parts=nop,persistent=persistent)
            .flatten_by_key(lambda x: dispatch(iter(x)))
        )

    def combine_by_key(
        self, creator_func, combiner_func, merger_function, total_parts=-1,persistent=False
    ):
        ...
        ret = (
            ...
            .partition_by(num_of_partitions=total_parts,persistent=persistent)
            .map_partitions(merge_partition)
        )

        return ret

    def reduce_by_key(self, func,persistent=False):
        ...
        """
        return self.combine_by_key((lambda x: x), func, func,persistent=persistent)

    def count_by_key(self, as_dict=False,persistent=False):
        """Count by key.
        ...
        :return: A new DDS with data set of list of tuples
                 (element, occurrence).
        """
        return self.map(lambda x: x[0]).count_by_value(as_dict=as_dict,persistent=persistent)

    def sort_by_key(
        self, ascending=True, num_of_parts=None, key_func=lambda x: x, persistent=False
    ):
        """Sort by key.
        ...
        partitioned = DDS().load(col_parts, -1, persistent=persistent).partition_by(range_partitioner,persistent=persistent)
        return partitioned.map_partitions(sort_partition)

    def group_by_key(self, num_of_parts=-1,persistent=False):
        ...
        return self.combine_by_key(
            _create, _merge, _combine, total_parts=num_of_parts, persistent=persistent
        )
