...
class IPartitionGenerator(DataClayObject):  # pylint: disable=too-few-public-methods
...
class IteratorLoader(IPartitionGenerator):  # pylint: disable=R0903
    ...
    def __init__(self, iterable, start, end):
        ...
        :returns: None.
        """
        logger.info("ITERATOR LOADER YUHUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU")
        print("MAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAHA")
        super().__init__()
        self.iterable = iterable
        print("ITERAAAAALBE????, %s" , self.get_iterable())
        self.start = start
        self.end = end

    @activemethod
    def get_iterable(self):
        print("PERO AQUI ARRIBA????????????")
